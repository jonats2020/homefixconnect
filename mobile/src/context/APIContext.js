...
export const APIProvider = ({ children }) => {
  ...
  const jobs = {
    ...
    assignContractor: async (jobId, bidId) => {
      try {
        ...
        return { 
          success: false, 
          message: error.response?.data?.message || 'Failed to assign contractor' 
        };
      }
    }
  };
  ...
  const chat = {
    ...
    getOrCreateConversation: async (userId) => {
      try {
        const response = await api.post('/api/chats/conversation', { userId });
        return { success: true, data: response.data };
      } catch (error) {
  ...
  const users = {
    ...
    searchContractors: async (query) => {
      try {
        const response = await api.get(`/api/users/search/contractors?q=${query}`);
        return { success: true, data: response.data };
      } catch (error) {
        console.error('Search contractors error:', error);
        return { 
          success: false, 
          message: error.response?.data?.message || 'Failed to search contractors' 
        };
      }
    }
  };
  ...
  const ratings = {
    ...
    getJobRatings: async (jobId) => {
      try {
        ...
        return { 
          success: false, 
          message: error.response?.data?.message || 'Failed to fetch job ratings' 
        };
      }
    }
  };
